...
namespace Skybrud.Umbraco.GridData.Converters.Umbraco {
    ...
    /// </summary>
    public class MediaGridConverter : IGridConverter {
        ...
        public virtual bool ConvertControlValue(GridControl control, JToken token, out IGridControlValue value) {
            ...
            value = IsMediaEditor(control.Editor)
                ? GridControlMediaValue.Parse(control, token as JObject)
                : null;
            return value != null;
        }
        ...
        public virtual bool ConvertEditorConfig(GridEditor editor, JToken token, out IGridEditorConfig config) {
            config = IsMediaEditor(editor)
                ? GridEditorMediaConfig.Parse(editor, token as JObject)
                : null;
            return config != null;        
        }
        ...
        public virtual bool GetControlWrapper(GridControl control, out GridControlWrapper wrapper) {
            ...
            wrapper = IsMediaEditor(control.Editor)
                ? control.GetControlWrapper<GridControlMediaValue, GridEditorMediaConfig>()
                : null;
            return wrapper != null;
        }
